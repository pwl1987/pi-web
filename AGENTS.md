...
本项目在**中文语境**下，对所有 AI 代理与代码产出执行严格本地化规则：

- **用户可见文案（状态 / 提示 / 交互回复 / 系统提示）**：必须全部使用中文，**绝对禁止**输出未经翻译的英文状态或提示。用户可见字符串一律走 `lib/i18n/zh.ts`（本仓库已有零依赖 i18n，zh/en 双语）。
- **待办事项（TODO）/ 任务列表 / 进度追踪 / 执行步骤**：保持全中文表达。
- **代码注释**：新写的注释使用中文。
- **变量命名（如适用）**：内部标识符（变量 / 函数 / 类名）保留英文是行业惯例，且改动会破坏编译与跨文件引用，故按"如适用"处理——仅当用户显式要求时才中文化；不要为本地化而将标识符改成拼音或中文。
- **最终回复**：保持全中文，不夹带裸英文短语。
