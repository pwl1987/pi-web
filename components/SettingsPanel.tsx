...
export function SettingsPanel({ onClose, onOpenModels, onOpenSkills, onOpenPlugins, onOpenExtensions, onOpenAgents }: Props) {
  ...
  useEffect(() => {
    fetch("/api/settings").then(r => r.json()).then(setPiSettings).catch(() => {});
  ...
  const selectedModelKey = piSettings?.defaultProvider && piSettings?.defaultModel
  ...
  return (
    ...
    >
      <div style={{
        width: "min(520px, calc(100vw - 32px))", maxHeight: "calc(100dvh - 32px)",
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 18px", borderBottom: "1px solid var(--border)", flexShrink: 0 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            {saving && <span style={{ fontSize: 11, color: "var(--text-dim)" }}>{t("settings.saving")}</span>}
            {savedFlash && <span style={{ fontSize: 11, color: "var(--accent)" }}>✓ {t("settings.saved")}</span>}
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "0" }}>
            ...
            <Row label={t("settings.defaultModel")} hint={t("settings.defaultModelHint")}>
              <select
                ...
                onChange={(e) => {
                  ...
                  // Save both together
                  void saveField("__batch", { defaultProvider: provider || null, defaultModel: modelId || null });
                }}
                style={selectStyle}
              >
                <option value="">—</option>
          ...
          {piSettings && (
            <Row label={t("settings.defaultThinking")}>
              <select value={piSettings.defaultThinkingLevel} onChange={(e) => updatePi("defaultThinkingLevel", e.target.value)} style={selectStyle}>
                {["auto", "off", "minimal", "low", "medium", "high", "xhigh"].map(l => <option key={l} value={l}>{l}</option>)}
              </select>
            </Row>
          )}
          ...
          {/* Compaction toggle */}
          {piSettings && (
            <ToggleRow label={t("settings.compaction")} hint={t("settings.compactionHint")} checked={piSettings.compactionEnabled} onChange={(v) => updatePi("compactionEnabled", v)} />
          )}

          {/* Retry toggle */}
          {piSettings && (
            <ToggleRow label={t("settings.retry")} hint={t("settings.retryHint")} checked={piSettings.retryEnabled} onChange={(v) => updatePi("retryEnabled", v)} />
          )}

          {/* Steering / Follow-up mode */}
          {piSettings && (
            <Row label={t("settings.steering")}>
              <select value={piSettings.steeringMode} onChange={(e) => updatePi("steeringMode", e.target.value)} style={selectStyle}>
          ...
          {piSettings && (
            <Row label={t("settings.followUp")}>
              <select value={piSettings.followUpMode} onChange={(e) => updatePi("followUpMode", e.target.value)} style={selectStyle}>
                <option value="all">all</option>
                <option value="one-at-a-time">one-at-a-time</option>
              </select>
            </Row>
          ...
          {piSettings && (
            <Row label={t("settings.transport")}>
              <select value={piSettings.transport} onChange={(e) => updatePi("transport", e.target.value)} style={selectStyle}>
          ...
          {piSettings && (
            <Row label={t("settings.httpTimeout")}>
              <input type="number" value={piSettings.httpIdleTimeoutMs} onChange={(e) => updatePi("httpIdleTimeoutMs", parseInt(e.target.value) || 0)} style={inputStyle} />
            </Row>
          )}
          ...
          {piSettings && (
            <Row label={t("settings.shellPath")}>
              <input type="text" value={piSettings.shellPath} onChange={(e) => updatePi("shellPath", e.target.value)} style={inputStyle} placeholder="/bin/bash" />
            </Row>
          )}
          {piSettings && (
            <Row label={t("settings.shellPrefix")}>
              <input type="text" value={piSettings.shellCommandPrefix} onChange={(e) => updatePi("shellCommandPrefix", e.target.value)} style={inputStyle} />
            </Row>
          )}
          {piSettings && (
            <Row label={t("settings.npmCommand")}>
              <input type="text" value={(piSettings.npmCommand ?? []).join(" ")} onChange={(e) => updatePi("npmCommand", e.target.value.split(" ").filter(Boolean))} style={inputStyle} placeholder="npm" />
            </Row>
          )}
          ...
          {piSettings && (
            <Row label={t("settings.trust")}>
              <select value={piSettings.defaultProjectTrust} onChange={(e) => updatePi("defaultProjectTrust", e.target.value)} style={selectStyle}>
          ...
          {piSettings && (
            <ToggleRow label={t("settings.hideThinking")} checked={piSettings.hideThinkingBlock} onChange={(v) => updatePi("hideThinkingBlock", v)} />
          )}
          {piSettings && (
          ...
          <ManagementButton label={t("settings.openModels") + " (models.json)"} onClick={() => { onClose(); onOpenModels(); }} />
          ...
          <Row label={t("settings.theme")}>
            <div style={{ display: "flex", gap: 0, borderRadius: 7, border: "1px solid var(--border)", overflow: "hidden" }}>
              <button onClick={() => { if (isDark) toggleTheme(); }} style={{ ...segBtnStyle, background: !isDark ? "var(--bg-selected)" : "none", color: !isDark ? "var(--text)" : "var(--text-muted)", borderRight: "1px solid var(--border)" }}>{t("settings.themeLight")}</button>
              <button onClick={() => { if (!isDark) toggleTheme(); }} style={{ ...segBtnStyle, background: isDark ? "var(--bg-selected)" : "none", color: isDark ? "var(--text)" : "var(--text-muted)" }}>{t("settings.themeDark")}</button>
            </div>
          </Row>
          ...
          <Row label={t("settings.language")}>
            <div style={{ display: "flex", gap: 0, borderRadius: 7, border: "1px solid var(--border)", overflow: "hidden" }}>
              <button onClick={() => setLocale("en")} style={{ ...segBtnStyle, background: locale === "en" ? "var(--bg-selected)" : "none", color: locale === "en" ? "var(--text)" : "var(--text-muted)", borderRight: "1px solid var(--border)" }}>{t("settings.languageEn")}</button>
              <button onClick={() => setLocale("zh")} style={{ ...segBtnStyle, background: locale === "zh" ? "var(--bg-selected)" : "none", color: locale === "zh" ? "var(--text)" : "var(--text-muted)" }}>{t("settings.languageZh")}</button>
            </div>
          </Row>

          {/* Sound */}
          <ToggleRow label={t("settings.sound")} hint={t("settings.soundHint")} checked={soundEnabled} onChange={onSoundToggle} />
        </div>
      </div>
    </div>
  );
...
const selectStyle: React.CSSProperties = {
  ...
  outline: "none", fontFamily: "var(--font-mono)", minWidth: 120,
};
...
const segBtnStyle: React.CSSProperties = {
  padding: "4px 12px", fontSize: 12, fontWeight: 500, cursor: "pointer", border: "none",
};

function SectionHeader({ label }: { label: string }) {
  return (
    <div style={{ padding: "14px 18px 4px", fontSize: 11, fontWeight: 700, color: "var(--text-dim)", textTransform: "uppercase", letterSpacing: 0.5, borderTop: "1px solid var(--border)" }}>
      {label}
    </div>
  );
}
...
function ToggleRow({ label, hint, checked, onChange }: { label: string; hint?: string; checked: boolean; onChange: (v: boolean) => void }) {
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 18px" }}>
      ...
      <button onClick={() => onChange(!checked)} style={{ width: 36, height: 20, borderRadius: 10, cursor: "pointer", padding: 0, border: "none", position: "relative", background: checked ? "var(--accent)" : "var(--bg-hover)", transition: "background 0.15s" }}>
  ...
  return (
    <div style={{ padding: "4px 18px" }}>
      <button onClick={onClick} style={{
        ...
        cursor: "pointer", borderRadius: 7, background: "var(--bg-hover)", border: "1px solid var(--border)",
        color: "var(--text)", textAlign: "left", display: "flex", alignItems: "center", justifyContent: "space-between",
      }}>
        {label}
