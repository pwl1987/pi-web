 ...
 */
export function TodoSidebar({ sessionId, open, onToggle }: {
      ...
      {open && (
        <div style={{
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "4px 0" }}>
            {!hasTasks ? (
              <div style={{ padding: "24px 16px", fontSize: 12, color: "var(--text-dim)", textAlign: "center", lineHeight: 1.6 }}>
                {t("todo.empty")}
              </div>
            ) : (
              <>
    ...
    <div style={{ marginBottom: 2 }}>
      <button
        ...
        style={{
          ...
          color: accent ? "var(--accent)" : "var(--text-dim)", textAlign: "left",
        }}
      >
      ...
      {!collapsed && tasks.map((task) => (
        <div key={task.id} style={{
          display: "flex", alignItems: "flex-start", gap: 7, padding: "5px 12px 5px 20px",
        }}>
          <span style={{
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            {task.description && !muted && (
              <div style={{ fontSize: 10, color: "var(--text-muted)", marginTop: 1 }}>{task.description}</div>
            )}
            {task.activeForm && accent && (
              <div style={{ fontSize: 10, color: "var(--accent)", fontStyle: "italic", marginTop: 1 }}>⟳ {task.activeForm}</div>
            )}
            {task.blockedBy && task.blockedBy.length > 0 && !muted && (
              <div style={{ fontSize: 10, color: "var(--text-dim)", marginTop: 1 }}>⛔ {task.blockedBy.join(", ")}</div>
            )}
          </div>
        </div>
      ))}
