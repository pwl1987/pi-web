...
test("maps pi file-format toolCall to internal format", async () => {
  ...
  const msg = {
    role: "assistant",
    content: [
      { type: "toolCall", id: "tc-1", name: "write_file", arguments: { path: "/a.ts", content: "x" } },
    ],
  };
...
test("does not re-normalize already-correct toolCall blocks", async () => {
  ...
  const msg = {
    role: "assistant",
    content: [
      { type: "toolCall", toolCallId: "tc-1", toolName: "read", input: { path: "/b" } },
    ],
  };
  const result = normalizeToolCalls(msg);
  const block = result.content[0];
...
test("prefers internal field names when both exist", async () => {
  ...
  const msg = {
    ...
    content: [{
      type: "toolCall",
      id: "file-id", name: "file-name", arguments: { a: 1 },
      toolCallId: "internal-id", toolName: "internal-name", input: { b: 2 },
    }],
  };
  const result = normalizeToolCalls(msg);
  const block = result.content[0];
